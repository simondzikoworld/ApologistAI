...
export default function ConversationSidebar({ conversations, activeId, onNew, onDelete, isPro }: Props) {
  ...
  return (
    <aside className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 w-64 shrink-0">
      {/* Logo + new chat */}
      <div className="px-4 pt-5 pb-3 border-b border-slate-200 dark:border-slate-800">
        ...
        <button
          onClick={onNew}
          className="w-full flex items-center gap-2 px-3 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold transition-colors"
        >
          <PlusIcon />
          New conversation
        </button>
